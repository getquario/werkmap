...
// them, which is the order an `Xf` holds the ids in. Alignment is not a table,
// so it is written apart.
const APPLIES = ["applyNumberFormat", "applyFont", "applyFill", "applyBorder"];
...
const xfXml = (xf) => {
  ...
  const applies = APPLIES.filter((_flag, id) => xf[id] !== 0)
    .map((flag) => ` ${flag}="1"`)
    .join("");
