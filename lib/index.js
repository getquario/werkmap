...
// One entry's two headers. From the version onward the local header repeats
// the central record field for field, two bytes earlier for want of a
// `version made by`, so the span is written once and copied across.
/**
 * @param {number} method @param {number} sum @param {Uint8Array} packed
 * @param {Uint8Array} raw @param {Uint8Array} name @param {number} offset
 */
const headers = (method, sum, packed, raw, name, offset) => {
  const record = new DataView(new ArrayBuffer(46));
  record.setUint32(0, 0x02014b50, true);
  record.setUint16(4, 20, true);
  record.setUint16(6, 20, true);
  record.setUint16(8, 0x0800, true); // the name is UTF-8
  ...
  const central = new Uint8Array(record.buffer);
  const head = new Uint8Array(30);
  new DataView(head.buffer).setUint32(0, 0x04034b50, true);
  head.set(central.subarray(6, 32), 4);
  return [head, central];
};
...
/**
 * @typedef {[name: string, body: Uint8Array, store?: boolean]} Entry
 */
...
const zip = async (entries) => {
  ...
  for (const [path, raw, store] of entries) {
    const [packed, method] = store ? [raw, STORED] : [await deflate(raw), DEFLATED];
    const name = utf8(path);
    const [head, record] = headers(method, crc32(raw), packed, raw, name, offset);
...
// Every `<t>` this writer emits carries `xml:space="preserve"`, not only the
// ones whose text needs it: one rule is easier to trust than a predicate over
// whitespace.
 ...
 * the cell font. A run's font is a cell font but for the tag names inside
 * `rPr`, where `name` is spelled `rFont`.
 * @param {unknown} run
 * @param {string} at
 */
const richRun = (run, at) => {
  const { text, font } = requireRecord(run, at, "a { text, font } run");
  const body = `<t xml:space="preserve">${esc(requireString(text, `${at}.text`))}</t>`;
  const properties = absent(font)
    ? ""
    : `<rPr>${fontXml(/** @type {Record<string, unknown>} */ (font), `${at}.font`).replace(/<name val=/, "<rFont val=")}</rPr>`;
 ...
 */
const xfXml = (xf) => {
  const ids = `numFmtId="${xf[0]}" fontId="${xf[1]}" fillId="${xf[2]}" borderId="${xf[3]}"`;
  // In the order `CT_Xf` lists the flags, which is the order an `Xf` holds the
  // ids in. Alignment is not a table, so it is written apart.
  const applies =
    (xf[0] ? ' applyNumberFormat="1"' : "") +
    (xf[1] ? ' applyFont="1"' : "") +
    (xf[2] ? ' applyFill="1"' : "") +
    (xf[3] ? ' applyBorder="1"' : "");
  if (xf[4] === "") return `<xf ${ids} xfId="0"${applies}/>`;
  return `<xf ${ids} xfId="0"${applies} applyAlignment="1"><alignment${xf[4]}/></xf>`;
};
...
const stylesheet = () => {
  ...
  const formats = table([["", /** @type {Xf} */ ([0, 0, 0, 0, ""])]]);
  ...
  const formatId = (code, where) => {
    ...
    const seen = BUILT_IN.get(code) ?? numFmts.get(code);
    if (seen !== undefined) return seen;
  ...
  return {
    ...
    intern(style, where) {
      ...
      /** @type {Xf} */
      const record = [
        formatId(style.numberFormat, `${where}.numberFormat`),
        optional(style.font, fonts, (font) => [
          JSON.stringify(font),
          () => `<font>${fontXml(font, `${where}.font`)}</font>`,
        ]),
        optional(style.fill, fills, () => {
        ...
        optional(style.border, borders, (border) => [
          JSON.stringify(border),
          () =>
            `<border>${SIDES.map((side) => sideXml(side, border[side], `${where}.border.${side}`)).join("")}<diagonal/></border>`,
        ]),
        absent(style.alignment)
          ? ""
          : alignmentXml(
 ...
 * @typedef {number | { top: number, right: number, bottom: number, left: number }} PrintMargin
 * @typedef {{ margin?: PrintMargin, size?: string, orientation?: string, fit?: boolean,
 ...
 * codes that change the look before it. An ampersand in author text doubles,
 * so none of it reads back as a code.
 * @param {unknown} part
 * @param {string} at
 * @param {Look} look
 */
const furniturePart = (part, at, look) => {
  ...
  return codes + requireString(record.text, `${at}.text`).replace(/&/g, "&&");
};
...
const furniture = (text, at) => {
  ...
  // In the order the format reads them.
  return (
    (sections.left === undefined ? "" : "&L" + sectionText(sections.left, `${at}.left`)) +
    (sections.center === undefined ? "" : "&C" + sectionText(sections.center, `${at}.center`)) +
    (sections.right === undefined ? "" : "&R" + sectionText(sections.right, `${at}.right`))
  );
};
...
/** @param {number} points */
const inches = (points) => (points / POINTS_PER_INCH).toFixed(3);

/** @param {PrintMargin | undefined} margin in points */
const marginsXml = (margin) => {
  ...
  const { top, right, bottom, left } =
    typeof margin === "number"
      ...
      : margin;
  return (
    `<pageMargins left="${inches(left)}" right="${inches(right)}"` +
    ` top="${inches(top)}" bottom="${inches(bottom)}"` +
...
const requireOptions = (options) => {
  // A stray key is refused rather than dropped: a caller asking for a row
  // attribute would otherwise get no error and no attribute.
  const said = requireRecord(options ?? {}, "row: options", "{ level, hidden, collapsed }");
  const stray = Object.keys(said).find(
    (name) => name !== "level" && name !== "hidden" && name !== "collapsed",
  );
...
const requireOutline = (level) => {
  if (level !== 0 && !isIndex(level, MAX_LEVEL))
    throw RangeError(
...
const textIndex = (sst, value, at) => {
  if (typeof value === "string")
    return sst.intern("s" + value, () => `<si><t xml:space="preserve">${esc(value)}</t></si>`);
  if (Array.isArray(value))
    return sst.intern(
      richKey(value, at),
      () => "<si>" + value.map((run, index) => richRun(run, `${at}[${index}]`)).join("") + "</si>",
    );
 ...
 * reads back as a day rather than as the number underneath it. A style that is
 * not an object passes through untouched, so the interning is still the one
...
const cellStyle = (value, declared) =>
  ...
  (absent(declared) || typeof declared === "object") &&
  absent(declared?.numberFormat)
 ...
 * @typedef {[id: number, row: number, col: number, width: number, height: number]} Picture
 */

/** @param {Range} range */
const rangeRef = ([top, left, bottom, right]) =>
  `${letters(left)}${top}:${letters(right)}${bottom}`;
...
const requireFilter = (range, rows, columns) => {
  ...
  if (bottom < top) throw RangeError(`filter: bottom ${bottom} is above top ${top}`);
  if (right < left) throw RangeError(`filter: right ${right} is left of left ${left}`);
...
/** @param {unknown} value @param {string} where */
const requireInset = (value, where) => {
  const points = requireNumber(value, where);
  if (points < 0) throw RangeError(`${where} cannot be negative, got ${points}`);
};

/** @param {unknown} margin */
const requireMargin = (margin) => {
  if (typeof margin !== "object" || margin === null) return requireInset(margin, "print: margin");
  const keys = Object.keys(margin);
  if (keys.some((key) => !MARGIN_SIDES.includes(key)))
    throw RangeError(
      `print: margin takes top, right, bottom and left, got ${JSON.stringify(keys)}`,
    );
  for (const side of MARGIN_SIDES)
    requireInset(/** @type {any} */ (margin)[side], `print: margin.${side}`);
};
...
const worksheet = (name, styles, sst, knows) => {
  ...
  return {
    ...
    merge(row, at, width) {
      ...
      const right = mergeEnd(width, at);
      const clash = merges.find(
        (other) => row <= other[2] && row >= other[0] && at <= other[3] && right >= other[1],
      );
      if (clash !== undefined)
        throw RangeError(
          `merge: the range ${rangeRef([row, at, row, right])} overlaps ${rangeRef(clash)}`,
        );
      merges.push([row, at, row, right]);
    ...
    place(id, at) {
      ...
      pictures.push([/** @type {number} */ (id), row, col, width, height]);
    },
...
const drawingXml = (pictures, media) =>
  ...
  pictures
    .map((picture, index) => {
      const cx = Math.round(picture[3] * EMU);
      const cy = Math.round(picture[4] * EMU);
      return (
        `<xdr:oneCellAnchor editAs="oneCell">` +
        `<xdr:from><xdr:col>${picture[2] - 1}</xdr:col><xdr:colOff>0</xdr:colOff><xdr:row>${picture[1] - 1}</xdr:row><xdr:rowOff>0</xdr:rowOff></xdr:from>` +
        `<xdr:ext cx="${cx}" cy="${cy}"/>` +
        `<xdr:pic>` +
 ...
 * @param {ReadonlyArray<[Uint8Array, string]>} media
 */
const contentTypesXml = (sheets, drawings, media) => {
  ...
  const defaults = [...new Set(media.map((each) => each[1]))]
    // Both formats this writer takes spell their extension the same as their
    // IANA subtype, so the content type is the extension.
    .map((format) => `<Default Extension="${format}" ContentType="image/${format}"/>`)
    .join("");
...
/**
 * @param {[string?, string?, string?, string?]} meta title, creator, subject, description
 */
const coreXml = ([title, creator, subject, description]) =>
  DECLARATION +
 ...
 *
 * The sheet reference is always quoted, so a name holding a space or a symbol
 * needs no special case; an apostrophe in the name closes the quote early
 * unless it doubles.
 *
...
const definedNamesXml = (sheets) => {
  const names = sheets
    .map((sheet, index) =>
      sheet.titles === 0
        ? ""
        : `<definedName name="_xlnm.Print_Titles" localSheetId="${index}">${esc(`'${sheet.name.replace(/'/g, "''")}'`)}!$1:$${sheet.titles}</definedName>`,
    )
    .join("");
 ...
 * @typedef {ReturnType<typeof worksheet>} Sheet
 */
 ...
 * @param {Part} part
 * @param {ReadonlyArray<Sheet>} sheets
 * @param {ReadonlyArray<Sheet>} drawings
 */
const sheetParts = (part, sheets, drawings) => {
  for (const [index, sheet] of sheets.entries()) {
    const at = drawings.indexOf(sheet);
...
const drawingParts = (part, drawings, media) => {
  for (const [at, each] of drawings.entries()) {
    const used = [...new Set(each.pictures.map(([id]) => id))];
    part(
      `xl/drawings/drawing${at + 1}.xml`,
      drawingXml(
        each.pictures,
        each.pictures.map(([id]) => used.indexOf(id)),
      ),
    );
    part(
      ...
      DECLARATION +
        ...
        used
          .map(
            (id, index) =>
              `<Relationship Id="rId${index + 1}" Type="${REL}/image" Target="../media/image${id + 1}.${media[id][1]}"/>`,
          )
          .join("") +
        `</Relationships>`,
    );
...
export const workbook = (meta = {}) => {
  ...
  return {
    ...
    image(bytes, format) {
      ...
      const seen = media.findIndex((each) => each[1] === format && same(each[0], bytes));
    ...
    async bytes() {
      ...
      for (const [index, each] of media.entries())
        part(`xl/media/image${index + 1}.${each[1]}`, each[0], true);
