...
// count of characters of the workbook's default font, which is the number
// Excel's own column-width box shows. Pixels would read more naturally beside
// `place`, but the conversion runs through the default font's maximum digit
// width -- 7 for Calibri 11 -- and any cell here may name a font of its own,
// so that constant would be quietly wrong for most workbooks. Taking the
// format's unit raw invents no number, and the caller's measurement is
...
/**
 * A `<col>` per column that was given a width, in column order, and nothing at
 * all when none was. One element apiece rather than a `min`/`max` run: runs
 * save bytes on one of the smallest parts here, and buy a rule -- a run may
 * never span a column left unset -- that the sparse case would break first.
 *
...
// ask it, of a row and of a column, so the sentence lives here rather than
// twice.
 ...
 * A range whose bottom row was never written is a caller's bug, exactly as it
 * is for a merge, so this takes the row count to check it against.
 *
 * @param {unknown} range
...
const worksheet = (name, styles, sst, knows) => {
  ...
  return {
    ...
    filter(range) {
      ...
      // `_xlnm._FilterDatabase` defined name, and this writes none. Measured
      // rather than reasoned about, in LibreOffice: it opens a file carrying
      // only this element, keeps the filter, and writes that name itself on
      // save -- so the name is that reader's bookkeeping. Excel's own handling
      // is untested, and README.md says so where a caller reads it.
      filtered =
        range === null
          ? ""
