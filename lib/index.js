...
const worksheet = (name, styles, sst, knows) => {
  ...
  return {
     ...
     */
    print(setup) {
      requireRecord(setup, "print", "a setup object");
      if (setup.margin !== undefined) requireMargin(setup.margin);
      if (setup.size !== undefined) requirePaper(setup.size);
      if (setup.orientation !== undefined) requireOrientation(setup.orientation);
...
/** @param {ReadonlyArray<{ name: string }>} sheets */
