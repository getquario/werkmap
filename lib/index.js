...
const requireLevel = (options) => {
  const level =
    options === undefined ? 0 : (requireRecord(options, "row: options", "{ level }").level ?? 0);
...
const worksheet = (name, styles, sst, knows) => {
  ...
  return {
    ...
    row(cells, options) {
      ...
      rows.push(`<row r="${at}"${outline}>${body}</row>`);
      return at;
    },
    ...
    xml(drawing) {
      ...
      return (
        ...
        // The format requires a default row height beside the level count;
        // 15 is Excel's own for the default 11-point font.
        (deepest === 0
          ? ""
          : `<sheetFormatPr defaultRowHeight="15" outlineLevelRow="${deepest}"/>`) +
        colsXml(columns) +
